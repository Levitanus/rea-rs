...
/// via cc crate). See the C++ source file for a detailled explanation.
#[cfg(target_family = "unix")]
...
/// Compiles C++ glue code. This is necessary to interact with those parts of
/// the REAPER C++ API that use pure virtual interface classes and therefore
/// the C++ ABI.
...
mod codegen {
    ...
    pub mod stage_one {
        ...
        impl ParseCallbacks for CustomParseCallbacks {
            fn int_macro(&self, name: &str, _value: i64) -> Option<IntKind> {
                ...
                // The following constants were interpreted as signed integers
                // before but I changed them to unsigned:
                // - VK_: Although declared as signed in winapi-rs, I think it
                //   should be unsigned because MapVirtualKey function takes an
                //   unsigned integer and none of the VK_ constants are defined
                //   as negative integer.
                // - SWP_: Unsigned because SetWindowPos() expects unsigned.
                //   winapi-rs conforms.
                //
                // The following flags stay u32 although the APIs expect i32:
                // - UNDO_STATE_* (used as bitmask, UNDO_STATE_ALL doesn't fit
                //   into i32)
                None
            }
        ...
        pub fn generate_bindings() {
            ...
            let builder = bindgen::Builder::default()
                ...
                // If we activate layout tests, we would have to regenerate at
                // each build because tests will fail on Linux
                // if generated on Windows and vice versa.
                .layout_tests(false)
                ...
                .module_raw_line(
                    "root",
                    include_str!("src/manual_bindings.rs"),
                )
                ...
                .allowlist_type("project_config_extension_t")
    ...
    pub mod stage_two {
        ...
        use syn::token::{
            And, Colon, Extern, Fn, Paren, Pub, SelfValue, Unsafe,
        };
        use syn::{
            Abi, Block, Expr, ExprCall, ExprPath, FnArg, ForeignItem,
            ForeignItemFn, ForeignItemStatic, GenericArgument, Ident,
            ImplItem, ImplItemMethod, Item, ItemForeignMod, ItemMod, Pat,
            PatIdent, PatType, Path, PathArguments, PathSegment, Receiver,
            Signature, Type, TypeBareFn, VisPublic, Visibility,
        };

        /// For these functions exposed by REAPER the function pointers need to
        /// use `extern "system` instead of `extern "C"`, probably
        /// because they are forwarded directly to Win32 calls. This
        /// uses the "stdcall" calling convention when talking to
        ...
        ///
        /// For all the functions in this set, the generator will create
        /// `Swell` methods which are enabled on Windows only and just
        /// delegate to the Windows-native counterparts. This is
        /// convenient because then you just write your UI code using the
        /// `Swell` struct and it will work on Windows as well. It
        /// works because SWELL has been designed to imitate the
        /// Windows API exactly (well, more or less).
        ///
        /// This set doesn't include functions which have parameters where the
        /// character encoding matters. On Windows, they have either an
        /// `A` (ANSI encoding) or a `W` suffix (wide =
        /// UTF-16 encoding). Previously, we just delegated to the `A`
        /// functions here, but of course this didn't work at all with
        /// non-ANSI characters. However, simply delegating to the `W`
        /// functions was also not an option because first, the signature
        /// is different (`W` functions take strings as `*const u16` instead of
        /// `*const i8` because they are encoded as UTF-16). This is
        /// now implemented manually in `SwellImpl`.
        ...
        /// This is a list of types that influence if a generated method will
        /// be marked as unsafe or not.
        ///
        /// If a method has no parameters or only parameters of such types in
        /// this list, it will not be marked as unsafe.
        ///
        /// Before this list existed, all non-pointer function parameter were
        /// considered safe. But that was not reliable because it
        /// sometimes accidently interpreted a parameter as not being a
        /// pointer while in fact it was one: It was a type alias that resolved
        /// to a pointer e.g. HWND.
        ...
        /// This is an include list of additional REAPER functions that don't
        /// need to be marked unsafe.
        ///
        /// Previously this has been
        static ADDITIONAL_SAFE_REAPER_FUNCTIONS: phf::Set<&'static str> =
            phf::phf_set![];

        /// This is an include list of additional SWELL functions that don't
        /// need to be marked unsafe.
        static ADDITIONAL_SAFE_SWELL_FUNCTIONS: phf::Set<&'static str> =
            phf::phf_set![];

        /// Generates `reaper.rs` and `swell.rs` from the previously generated
        /// `bindings.rs`
        ...
        /// Generates the `swell.rs` file from the previously generated
        /// `bindings.rs`
        ...
        /// Generates the token stream. All of this could also be done in a
        /// procedural macro but I prefer the code generation approach
        /// for now.
        fn generate_reaper_token_stream(
            fn_ptrs: &Vec<FnPtr>,
        ...
        /// procedural macro but I prefer the code generation approach
        /// for now.
        fn generate_swell_token_stream(
            fn_ptrs: &Vec<FnPtr>,
        ) -> proc_macro2::TokenStream {
            let Compartments {
                names,
            ...
            let windows_functions: Vec<_> = fn_ptrs
                .iter()
                .filter_map(|p| {
                    if !WIN32_SWELL_FUNCTIONS
                        .contains(p.name.to_string().as_str())
                    {
                        return None;
                    }
                    // It's important to use `extern "system"` here, otherwise
                    // the linker will complain about
                    // missing externals on Windows 32-bit.
        ...
        /// This also modifies some unsafety and ABI fields based on the given
        /// sets.
            ...
            Compartments {
                ...
                fn_ptr_signatures: fn_ptrs
                    ...
                    .map(|p| {
                        ...
                        let abi = if extern_system_functions
        ...
        fn parse_file(path: &str) -> syn::File {
            let mut rust_file =
                File::open(path).expect("Unable to open file to be parsed");
        ...
        /// Parses the names and signatures of the function pointers from
        /// `bindings.rs`.
        ...
        fn generate_method_body(ptr: &FnPtr) -> Block {
            let name = &ptr.name;
            let fn_ptr_call = generate_fn_ptr_call(
                &ptr.signature,
        ...
        /// Generates the body of a Swell method that just delegates to the
        /// corresponding Windows function instead of calling the
        /// function pointer (which wouldn't exist because on
        /// Windows SWELL is neither available nor necessary).
        fn generate_swell_windows_method_body(
            ptr: &FnPtr,
            fn_name: Ident,
        ) -> Block {
        ...
        /// Generates the actual function pointer call in the body of a method
        /// in `impl Reaper`
        fn generate_fn_ptr_call(
        ...
        /// Extracts the items of the given `bindings.rs` syntax tree and sub
        /// module of root that contains function pointers.
        ...
        /// Converts a syntax tree item which represents a REAPER function
        /// pointer to our convenience struct `ReaperFnPtr`
        fn map_to_fn_ptr(item: &ForeignItemStatic) -> FnPtr {
            ...
            let bare_fn = match &option_segment.arguments {
                PathArguments::AngleBracketed(a) => {
                    let generic_arg =
                        a.args.first().expect("Angle bracket must have arg");
                    match generic_arg {
                        GenericArgument::Type(Type::BareFn(bare_fn)) => {
                            bare_fn
                        }
        ...
        impl FnPtr {
            /// Returns if the function represented by this function pointer
            /// can be considered safe.
            ///
            /// Is used for omitting the `unsafe` wherever possible.
            fn is_safe(
                &self,
                additional_safe_functions: &phf::Set<&'static str>,
            ) -> bool {
                // If this function is explicitly mentioned on the given
                // include list, consider it as safe.
                if additional_safe_functions
                    .contains(self.name.to_string().as_str())
                {
                    return true;
                }
                // If all parameters of this function are on a whitelist of
                // "safe" types, consider it as safe.
                self.signature.inputs.iter().all(|a| match &a.ty {
                    Type::Path(p) => {
                        let quoted = quote::quote! { #p };
                        let path_without_spaces =
                            quoted.to_string().replace(" ", "");
