use crate::{
    raw, register_plugin_destroy_hook, GetSwellFunc,
    StaticExtensionPluginContext, StaticVstPluginContext,
};
...
///
/// As a result it can later be picked up e.g. by using
/// [`static_extension_plugin_context()`]. This is typically called by some
/// SWELL entry point in one of the plugin macros.
///
...
pub fn register_swell_function_provider(get_func: Option<GetSwellFunc>) {
    ...
    // On Linux Rust will get informed first about the SWELL function provider,
    // so we need to pass it on to the C++ side.
    #[cfg(target_os = "linux")]
    unsafe {
...
///
/// As a result it can later be picked up by using
/// [`static_vst_plugin_context()`]. This is typically called on Windows only
/// by some entry point in one of the plugin macros.
///
...
mod swell {
    ...
    /// On Linux/macOS this will contain the SWELL function provider after
    /// REAPER start.
    ...
    #[cfg(target_os = "linux")]
    extern "C" {
        /// On Linux, this function is implemented on C++ side
        /// (`swell-modstub-generic-custom.cpp`).
        ///
        /// This is supposed to be called by Rust as soon as it gets hold of
        /// the SWELL function provider via the Linux SWELL main entry
        /// point `SWELL_dllMain()`. Calling this give the C++ side of
        /// the plug-in the chance to initialize its SWELL function pointers as
        /// well, which is necessary for many use cases (e.g. creating
        /// dialog windows). This is only needed on Linux. On Windows
        /// we don't have SWELL and on macOS the mechanism of obtaining SWELL
        /// is different.
    ...
    /// On macOS, this function is called from Objective-C side
    /// (`swell-modstub-custom.mm`).
    ///
    /// It lets Rust know about the SWELL function provider. It's called on
    /// REAPER startup.
    #[cfg(target_os = "macos")]
    #[no_mangle]
    unsafe extern "C" fn register_swell_called_from_cpp(
        get_func: Option<crate::GetSwellFunc>,
    ) {
