...
/// [`create_cpp_to_rust_control_surface()`]. After registering the returned
/// C++ counterpart, REAPER will start invoking the callback methods.
///
/// # Design
///
/// ## Why do most methods here don't take `&mut self` as parameter?
///
/// **Short answer:** Because we follow the spirit of Rust here, which is to
/// fail fast and thereby prevent undefined behavior.
///
/// **Long answer:** Taking `self` as `&mut` in control surface methods would
/// give us a dangerous illusion of safety (safety as defined by Rust). It
/// would tell Rust developers "It's safe here to mutate the state of my
/// control surface struct". But in reality it's not safe. Not because of
/// multi-threading (ControlSurfaces methods are invoked by REAPER's main
/// thread only) but because of reentrancy. That can happen quite easily, just
/// think of this scenario: A track is changed, REAPER notifies us about it by
/// calling a ControlSurface method, thereby causing another change
/// in REAPER which in turn synchronously notifies our ControlSurface again
/// while our first method is still running ... and there you go: 2 mutable
/// borrows of `self`. In a Rust-only world, Rust's compiler wouldn't allow us
/// to do that. But Rust won't save us here because the call comes from
/// "outside". By not having a `&mut self` reference, developers are forced to
/// explicitly think about this scenario. One can use a `RefCell` along with
/// `borrow_mut()` to still mutate some control surface state and failing fast
/// whenever reentrancy happens - at runtime, by getting a panic. This is not
/// as good as failing fast at compile time but still much better than to run
/// into undefined behavior, which could cause hard-to-find bugs and crash
/// REAPER - that's the last thing we want! Panicking is not so bad. We can
/// catch it before it reaches REAPER and therefore let REAPER continue
/// running. Ideally it's observed by the developer when he tests his plugin.
/// Then he can think about how to solve that issue. They might find out that
/// it's okay and therefore use some unsafe code to prevent the panic. They
/// might find out that they want to check for reentrancy by using
/// `try_borrow_mut()`. Or they might find out that they want to avoid this
/// situation by just deferring the event handling to the next main loop cycle.
///
...
/// Creates an `IReaperControlSurface` object on C++ side and returns a pointer
/// to it.
///
/// This function is provided because [`plugin_register()`] isn't going to work
/// if you just pass it a Rust struct as in
/// `reaper.plugin_register("csurf_inst", my_rust_struct)`. Rust structs can't
/// implement C++ virtual base classes.
///
/// **This function doesn't yet register the control surface!** The usual
/// REAPER C++ way to register a control surface still applies. You need to
/// pass the resulting pointer to [`plugin_register()`].
///
/// # Example
///
/// ```no_run
/// # let reaper = rea_rs_low::Reaper::default();
/// use rea_rs_low::{create_cpp_to_rust_control_surface, delete_cpp_control_surface, IReaperControlSurface};
/// use std::ffi::CString;
/// use std::ptr::NonNull;
...
/// # Cleaning up
///
/// If you register a control surface, you also must take care of unregistering
/// it at the end. This is especially important for VST plug-ins because they
/// live shorter than a REAPER session! **If you don't unregister the control
/// surface before the VST plug-in is destroyed, REAPER will crash** because it
/// will attempt to invoke functions which are not loaded anymore.
///
/// In order to avoid memory leaks, you also must take care of removing the C++
/// counterpart surface by calling [`delete_cpp_control_surface()`].
///
/// # Safety
///
/// This function is highly unsafe for all the reasons mentioned above. Better
/// use the medium-level API instead, which makes registering a breeze.
///
...
pub unsafe fn delete_cpp_control_surface(
    surface: NonNull<raw::IReaperControlSurface>,
) {
    crate::bindings::root::reaper_control_surface::delete_control_surface(
        surface.as_ptr(),
    );
}
...
extern "C" fn cpp_to_rust_IReaperControlSurface_Run(
    ...
    // "Decoding" the thin pointer is not necessary right now because we have a
    // static variable. However, we leave it. Might come in handy one day
    // to support multiple control surfaces (see https://users.rust-lang.org/t/sending-a-boxed-trait-over-ffi/21708/6)
...
extern "C" fn cpp_to_rust_IReaperControlSurface_SetSurfaceVolume(
    ...
    firewall(|| {
        unsafe { &*callback_target }.SetSurfaceVolume(trackid, volume)
    });
}
...
extern "C" fn cpp_to_rust_IReaperControlSurface_SetSurfaceSelected(
    ...
    firewall(|| {
        unsafe { &*callback_target }.SetSurfaceSelected(trackid, selected)
    });
}
...
extern "C" fn cpp_to_rust_IReaperControlSurface_SetSurfaceRecArm(
    ...
    firewall(|| {
        unsafe { &*callback_target }.SetSurfaceRecArm(trackid, recarm)
    });
}
...
extern "C" fn cpp_to_rust_IReaperControlSurface_GetTouchState(
    ...
    firewall(|| unsafe { &*callback_target }.GetTouchState(trackid, isPan))
        .unwrap_or(false)
}
...
extern "C" fn cpp_to_rust_IReaperControlSurface_Extended(
    ...
    firewall(|| {
        unsafe { &*callback_target }.Extended(call, parm1, parm2, parm3)
    })
    .unwrap_or(0)
}
