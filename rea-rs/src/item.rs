use crate::{
    ptr_wrappers::{MediaItem, MediaItemTake, MediaTrack, ReaProject},
    utils::string_from_buf,
    Color, KnowsProject, Position, Project, ProjectContext, ReaRsError,
    Reaper, ReaperResult, Take, TimeMode, Track, Volume, WithReaperPtr, GUID,
};
use int_enum::IntEnum;
use serde_derive::{Deserialize, Serialize};
use std::{ffi::CString, time::Duration};

#[derive(Debug, PartialEq)]
pub struct Item {
    project_ptr: Option<ReaProject>,
    ptr: MediaItem,
    should_check: bool,
}
impl WithReaperPtr for Item {
    type Ptr = MediaItem;
    fn get_pointer(&self) -> Self::Ptr {
        self.ptr
    }
    fn get(&self) -> Result<Self::Ptr, ReaRsError> {
        self.require_valid_2(&self.project())
    }
    fn make_unchecked(&mut self) {
        self.should_check = false;
    }
    fn make_checked(&mut self) {
        self.should_check = true;
    }
    fn should_check(&self) -> bool {
        self.should_check
    }
}
impl KnowsProject for Item {
    fn project(&self) -> Project {
        match self.project_ptr {
            Some(ptr) => Project::new(ProjectContext::Proj(ptr)),
            None => Project::new(ProjectContext::CurrentProject),
        }
    }
}
impl Item {
    pub(crate) fn from_raw_project_ptr(
        project_ptr: Option<ReaProject>,
        ptr: MediaItem,
    ) -> Self {
        Self {
            project_ptr,
            ptr,
            should_check: true,
        }
    }

    pub fn new<'a>(
        project: impl Into<Option<&'a Project>>,
        ptr: MediaItem,
    ) -> ReaperResult<Self> {
        let project_ptr = match project.into() {
            Some(p) => Some(p.get()?),
            None => None,
        };
        Ok(Self::from_raw_project_ptr(project_ptr, ptr))
    }

    pub fn track(&self) -> ReaperResult<Track> {
        let ptr = unsafe {
            Reaper::get().low().GetMediaItem_Track(self.get()?.as_ptr())
        };
        match MediaTrack::new(ptr) {
            None => panic!("Got null ptr! Maybe track is deleted?"),
            Some(ptr) => Ok(Track::new(self.project().get()?, ptr)),
        }
    }

    pub fn get_take(&self, index: usize) -> ReaperResult<Option<Take>> {
        let ptr = self.get_take_ptr(index)?;
        match ptr {
            Some(ptr) => Ok(Some(Take::new(ptr, Some(self))?)),
            None => Ok(None),
        }
    }

    fn get_take_ptr(
        &self,
        index: usize,
    ) -> ReaperResult<Option<MediaItemTake>> {
        let ptr = unsafe {
            Reaper::get()
                .low()
                .GetTake(self.get()?.as_ptr(), index as i32)
        };
        Ok(MediaItemTake::new(ptr))
    }

    pub fn active_take(&self) -> ReaperResult<Take> {
        let ptr = self
            .active_take_ptr()?
            .ok_or(ReaRsError::NullPtr("active take"))?;
        Take::new(ptr, Some(self))
    }

    fn active_take_ptr(&self) -> ReaperResult<Option<MediaItemTake>> {
        let ptr =
            unsafe { Reaper::get().low().GetActiveTake(self.get()?.as_ptr()) };
        Ok(MediaItemTake::new(ptr))
    }

    pub fn is_selected(&self) -> ReaperResult<bool> {
        Ok(unsafe {
            Reaper::get()
                .low()
                .IsMediaItemSelected(self.get()?.as_ptr())
        })
    }

    fn get_info_value(
        &self,
        category: impl Into<String>,
    ) -> ReaperResult<f64> {
        let category = category.into();
        Ok(unsafe {
            Reaper::get().low().GetMediaItemInfo_Value(
                self.get()?.as_ptr(),
                CString::new(category)?.as_ptr(),
            )
        })
    }

    pub fn position(&self) -> ReaperResult<Position> {
        Ok(self.get_info_value("D_POSITION")?.into())
    }
    pub fn length(&self) -> ReaperResult<Duration> {
        Ok(Duration::from_secs_f64(self.get_info_value("D_LENGTH")?))
    }
    pub fn end_position(&self) -> ReaperResult<Position> {
        Ok(self.position()? + self.length()?.into())
    }
    pub fn is_muted(&self) -> ReaperResult<bool> {
        Ok(self.get_info_value("B_MUTE")? != 0.0)
    }
    /// muted (ignores solo). setting this value will not affect
    /// [Item::solo_override].
    pub fn mute_actual(&self) -> ReaperResult<bool> {
        Ok(self.get_info_value("B_MUTE_ACTUAL")? != 0.0)
    }
    /// Basically, a way to solo particular item.
    ///
    /// This function will not override the same parameters on other items
    pub fn solo_override(&self) -> ReaperResult<ItemSoloOverride> {
        Ok(ItemSoloOverride::from_int(
            self.get_info_value("C_MUTE_SOLO")? as i32
        )
        .expect("can not convert value to item solo override"))
    }
    pub fn is_looped(&self) -> ReaperResult<bool> {
        Ok(self.get_info_value("B_LOOPSRC")? != 0.0)
    }
    pub fn all_takes_play(&self) -> ReaperResult<bool> {
        Ok(self.get_info_value("B_ALLTAKESPLAY")? != 0.0)
    }
    pub fn time_base(&self) -> ReaperResult<TimeMode> {
        Ok(
            TimeMode::from_int(self.get_info_value("C_BEATATTACHMODE")? as i32)
                .expect("Can not convert balue to time mode"),
        )
    }
    pub fn auto_stretch(&self) -> ReaperResult<bool> {
        Ok(self.get_info_value("C_AUTOSTRETCH")? != 0.0)
    }
    pub fn locked(&self) -> ReaperResult<bool> {
        Ok(self.get_info_value("C_LOCK")? != 0.0)
    }
    pub fn volume(&self) -> ReaperResult<Volume> {
        Ok(Volume::from(self.get_info_value("D_VOL")?))
    }
    pub fn snap_offset(&self) -> ReaperResult<Duration> {
        Ok(Duration::from_secs_f64(
            self.get_info_value("D_SNAPOFFSET")?,
        ))
    }
    pub fn fade_in(&self) -> ReaperResult<ItemFade> {
        let length =
            Duration::from_secs_f64(self.get_info_value("D_FADEINLEN")?);
        let curve = self.get_info_value("D_FADEINDIR")?;
        let shape = ItemFadeShape::from_int(
            self.get_info_value("C_FADEINSHAPE")? as i32,
        )
        .map_err(|_| ReaRsError::InvalidObject("Invalid fade-in shape"))?;
        let auto_fade_length = self.get_info_value("D_FADEINLEN_AUTO")? != 0.0;
        Ok(ItemFade {
            length,
            curve,
            shape,
            auto_fade_length,
        })
    }
    pub fn fade_out(&self) -> ReaperResult<ItemFade> {
        let auto_fade_length =
            self.get_info_value("D_FADEOUTLEN_AUTO")? != 0.0;
        let length = if auto_fade_length {
            Duration::from_secs_f64(self.get_info_value("D_FADEOUTLEN_AUTO")?)
        } else {
            Duration::from_secs_f64(self.get_info_value("D_FADEOUTLEN")?)
        };
        let curve = self.get_info_value("D_FADEOUTDIR")?;
        let shape = ItemFadeShape::from_int(
            self.get_info_value("C_FADEOUTSHAPE")? as i32,
        )
        .map_err(|_| ReaRsError::InvalidObject("Invalid fade-out shape"))?;
        Ok(ItemFade {
            length,
            curve,
            shape,
            auto_fade_length,
        })
    }
    pub fn group_id(&self) -> ReaperResult<usize> {
        Ok(self.get_info_value("I_GROUPID")? as usize)
    }
    /// Y-position (relative to top of track) in pixels
    pub fn y_pos_relative(&self) -> ReaperResult<usize> {
        Ok(self.get_info_value("I_LASTY")? as usize)
    }
    /// Y-position (relative to top of track) in pixels when track is in free
    /// mode
    ///
    /// 0 -> top of track, 1 -> bottom of track (will never be 1)
    pub fn y_pos_free_mode(&self) -> ReaperResult<f64> {
        self.get_info_value("F_FREEMODE_Y")
    }
    /// if None -> default.
    pub fn color(&self) -> ReaperResult<Option<Color>> {
        let raw = self.get_info_value("I_CUSTOMCOLOR")? as i32;
        if raw == 0 {
            return Ok(None);
        }
        Ok(Some(Color::from_native(raw & 0xffffff)))
    }
    /// Height in pixels, when track is in free mode
    ///
    /// 0 -> no height (will never be), 1 -> full track.
    pub fn height_free_mode(&self) -> ReaperResult<f64> {
        self.get_info_value("F_FREEMODE_H")
    }
    /// height in pixels
    pub fn height(&self) -> ReaperResult<usize> {
        Ok(self.get_info_value("I_LASTH")? as usize)
    }

    pub fn n_takes(&self) -> ReaperResult<usize> {
        Ok(unsafe {
            Reaper::get()
                .low()
                .GetMediaItemNumTakes(self.get()?.as_ptr())
                as usize
        })
    }

    fn get_info_string(
        &self,
        category: impl Into<String>,
        buf_size: usize,
    ) -> ReaperResult<String> {
        if buf_size < 2 {
            return Err(ReaRsError::InvalidObject(
                "buffer size must be at least 2",
            ));
        }
        let category = category.into();
        let mut buf = vec![0_i8; buf_size];
        let result = unsafe {
            Reaper::get().low().GetSetMediaItemInfo_String(
                self.get()?.as_ptr(),
                CString::new(category)?.as_ptr(),
                buf.as_mut_ptr(),
                false,
            )
        };
        if !result {
            return Err(ReaRsError::UnsuccessfulOperation(
                "Can not get value!",
            ));
        }

        string_from_buf(&buf)
    }
    pub fn notes(
        &self,
        buf_size: impl Into<Option<u32>>,
    ) -> ReaperResult<String> {
        let size = buf_size.into().unwrap_or(1024);
        self.get_info_string("P_NOTES", size as usize)
    }
    pub fn guid(&self) -> ReaperResult<GUID> {
        let guid_str = self.get_info_string("GUID", 50)?;
        Ok(GUID::from_string(guid_str)
            .expect("Can not convert GUID string to GUID"))
    }

    pub fn add_take(&mut self) -> ReaperResult<Take> {
        let ptr = unsafe {
            Reaper::get().low().AddTakeToMediaItem(self.get()?.as_ptr())
        };
        match MediaItemTake::new(ptr) {
            None => panic!("can not make Take"),
            Some(ptr) => Take::new(ptr, Some(self)),
        }
    }

    pub fn get_take_mut(
        &mut self,
        index: usize,
    ) -> ReaperResult<Option<Take>> {
        let ptr = self.get_take_ptr(index)?;
        match ptr {
            Some(ptr) => Ok(Some(Take::new(ptr, Some(self))?)),
            None => Ok(None),
        }
    }

    pub fn active_take_mut(&mut self) -> ReaperResult<Take> {
        let ptr = self
            .active_take_ptr()?
            .ok_or(ReaRsError::NullPtr("active take"))?;
        Take::new(ptr, Some(self))
    }

    pub fn set_position(
        &mut self,
        position: impl Into<Position>,
    ) -> ReaperResult<()> {
        unsafe {
            Reaper::get().low().SetMediaItemPosition(
                self.get()?.as_ptr(),
                position.into().into(),
                true,
            );
        }
        Ok(())
    }
    pub fn set_length(&mut self, length: Duration) -> ReaperResult<()> {
        unsafe {
            Reaper::get().low().SetMediaItemLength(
                self.get()?.as_ptr(),
                length.as_secs_f64(),
                true,
            );
        }
        Ok(())
    }
    pub fn set_end_position(
        &mut self,
        end_position: impl Into<Position>,
    ) -> ReaperResult<()> {
        let length: Duration = (end_position.into() - self.position()?).into();
        unsafe {
            Reaper::get().low().SetMediaItemLength(
                self.get()?.as_ptr(),
                length.as_secs_f64(),
                true,
            );
        }
        Ok(())
    }
    pub fn set_selected(&mut self, selected: bool) -> ReaperResult<()> {
        unsafe {
            Reaper::get()
                .low()
                .SetMediaItemSelected(self.get()?.as_ptr(), selected)
        }
        Ok(())
    }
    pub fn delete(self) -> ReaperResult<()> {
        unsafe {
            Reaper::get().low().DeleteTrackMediaItem(
                self.track()?.get()?.as_ptr(),
                self.get()?.as_ptr(),
            );
        }
        Ok(())
    }

    pub fn make_only_selected_item(&mut self) -> ReaperResult<()> {
        let mut pr = self.project();
        pr.select_all_items(false)?;
        self.set_selected(true)
    }

    pub fn split(
        self,
        position: impl Into<Position>,
    ) -> ReaperResult<ItemSplit> {
        let position: f64 = position.into().into();
        let ptr = unsafe {
            Reaper::get()
                .low()
                .SplitMediaItem(self.get()?.as_ptr(), position)
        };
        let ptr = match MediaItem::new(ptr) {
            None => {
                return Err(ReaRsError::InvalidObject(
                    "Can not split item, probably, bad position.",
                ))
            }
            Some(ptr) => ptr,
        };
        Ok(ItemSplit {
            left_ptr: self.get()?,
            right_ptr: ptr,
            project_ptr: self.project_ptr,
        })
    }

    pub fn update(&mut self) -> ReaperResult<()> {
        unsafe {
            Reaper::get()
                .low()
                .UpdateItemInProject(self.get()?.as_ptr())
        }
        Ok(())
    }

    pub fn move_to_track(&self, track_index: usize) -> ReaperResult<()> {
        let track = Track::from_index(&self.project(), track_index)?
            .ok_or(ReaRsError::InvalidObject("No track with given index!"))?;
        let track_ptr = track.get()?.as_ptr();
        let result = unsafe {
            Reaper::get()
                .low()
                .MoveMediaItemToTrack(self.get()?.as_ptr(), track_ptr)
        };
        match result {
            false => {
                Err(ReaRsError::UnsuccessfulOperation("Can not move item."))
            }
            true => Ok(()),
        }
    }

    fn set_info_value(
        &mut self,
        category: impl Into<String>,
        value: f64,
    ) -> ReaperResult<()> {
        let category = category.into();
        let result = unsafe {
            Reaper::get().low().SetMediaItemInfo_Value(
                self.get()?.as_ptr(),
                CString::new(category)?.as_ptr(),
                value,
            )
        };
        match result {
            true => Ok(()),
            false => {
                Err(ReaRsError::UnsuccessfulOperation("Can not set value."))
            }
        }
    }

    pub fn set_muted(&mut self, muted: bool) -> ReaperResult<()> {
        self.set_info_value("B_MUTE", muted as i32 as f64)
    }
    /// muted (ignores solo). setting this value will not affect
    /// [Item::solo_override].
    pub fn set_mute_actual(&mut self, mute: bool) -> ReaperResult<()> {
        self.set_info_value("B_MUTE_ACTUAL", mute as i32 as f64)
    }
    /// Basically, a way to solo particular item.
    ///
    /// This function will not override the same parameters on other items
    pub fn set_solo_override(
        &mut self,
        solo: ItemSoloOverride,
    ) -> ReaperResult<()> {
        self.set_info_value("C_MUTE_SOLO", solo.int_value() as f64)
    }
    pub fn set_looped(&mut self, looped: bool) -> ReaperResult<()> {
        self.set_info_value("B_LOOPSRC", looped as i32 as f64)
    }
    pub fn set_all_takes_play(
        &mut self,
        should_play: bool,
    ) -> ReaperResult<()> {
        self.set_info_value("B_ALLTAKESPLAY", should_play as i32 as f64)
    }
    pub fn set_time_base(&mut self, mode: TimeMode) -> ReaperResult<()> {
        self.set_info_value("C_BEATATTACHMODE", mode.int_value() as f64)
    }
    pub fn set_auto_stretch(&mut self, state: bool) -> ReaperResult<()> {
        self.set_info_value("C_AUTOSTRETCH", state as i32 as f64)
    }
    pub fn set_locked(&mut self, locked: bool) -> ReaperResult<()> {
        self.set_info_value("C_LOCK", locked as i32 as f64)
    }
    pub fn set_volume(&mut self, volume: Volume) -> ReaperResult<()> {
        self.set_info_value("D_VOL", volume.get())
    }
    pub fn set_snap_offset(&mut self, offset: Duration) -> ReaperResult<()> {
        self.set_info_value("D_SNAPOFFSET", offset.as_secs_f64())
    }
    pub fn set_fade_in(&mut self, fade: ItemFade) -> ReaperResult<()> {
        self.set_info_value("D_FADEINLEN", fade.length.as_secs_f64())?;
        self.set_info_value("D_FADEINDIR", fade.curve)?;
        self.set_info_value("C_FADEINSHAPE", fade.shape.int_value() as f64)?;
        self.set_info_value(
            "D_FADEINLEN_AUTO",
            fade.auto_fade_length as i32 as f64,
        )?;
        Ok(())
    }
    pub fn set_fade_out(&mut self, fade: ItemFade) -> ReaperResult<()> {
        self.set_info_value("D_FADEOUTLEN", fade.length.as_secs_f64())?;
        self.set_info_value("D_FADEOUTDIR", fade.curve)?;
        self.set_info_value("C_FADEOUTSHAPE", fade.shape.int_value() as f64)?;
        self.set_info_value(
            "D_FADEOUTLEN_AUTO",
            fade.auto_fade_length as i32 as f64,
        )?;
        Ok(())
    }
    pub fn set_group_id(&mut self, id: usize) -> ReaperResult<()> {
        self.set_info_value("I_GROUPID", id as f64)
    }
    /// Y-position (relative to top of track) in pixels when track is in free
    /// mode
    ///
    /// 0 -> top of track, 1 -> bottom of track (will never be 1)
    pub fn set_y_pos_free_mode(&mut self, y_pos: f64) -> ReaperResult<()> {
        assert!((0.0..1.0).contains(&y_pos));
        self.set_info_value("F_FREEMODE_Y", y_pos as f64)
    }
    /// Height in pixels, when track is in free mode
    ///
    /// 0 -> no height (will never be), 1 -> full track.
    pub fn set_height_free_mode(&mut self, height: f64) -> ReaperResult<()> {
        assert!((0.0..1.0).contains(&height));
        self.set_info_value("F_FREEMODE_H", height)
    }
    /// if None -> default.
    pub fn set_color(&mut self, color: Option<Color>) -> ReaperResult<()> {
        let color = match color {
            None => 0,
            Some(color) => color.to_native() | 0x1000000,
        };
        self.set_info_value("I_CUSTOMCOLOR", color as f64)
    }

    fn set_info_string(
        &self,
        category: impl Into<String>,
        value: impl Into<String>,
    ) -> ReaperResult<()> {
        let category = category.into();
        let value = value.into();
        let buf = CString::new(value)?.into_raw();
        let result = unsafe {
            Reaper::get().low().GetSetMediaItemInfo_String(
                self.get()?.as_ptr(),
                CString::new(category)?.as_ptr(),
                buf,
                true,
            )
        };
        match result {
            false => {
                Err(ReaRsError::UnsuccessfulOperation("Can not get value!"))
            }
            true => Ok(()),
        }
    }
    pub fn set_notes(&self, notes: impl Into<String>) -> ReaperResult<()> {
        let notes: String = notes.into();
        self.set_info_string("P_NOTES", notes)
    }
    pub fn set_guid(&self, guid: GUID) -> ReaperResult<()> {
        let guid_str = guid.to_string();
        self.set_info_string("GUID", guid_str)
    }
}

/// Holds two new items after [Item::split].
#[derive(Debug)]
pub struct ItemSplit {
    left_ptr: MediaItem,
    right_ptr: MediaItem,
    project_ptr: Option<ReaProject>,
}
impl ItemSplit {
    fn left(&mut self) -> Item {
        Item::from_raw_project_ptr(self.project_ptr, self.left_ptr)
    }
    fn right(&mut self) -> Item {
        Item::from_raw_project_ptr(self.project_ptr, self.right_ptr)
    }
    pub fn get(mut self) -> (Item, Item) {
        (self.left(), self.right())
    }
}

/// Item personal solo state.
///
/// If soloed -> other items and tracks will be not
/// overrided
#[repr(i32)]
#[derive(
    Debug, Copy, Clone, Eq, PartialEq, IntEnum, Serialize, Deserialize,
)]
pub enum ItemSoloOverride {
    Soloed = -1,
    NoOverride = 0,
    NotSoloed = 1,
}

/// Item FadeIn\FadeOut parameters.
#[derive(Debug, Clone, Copy, PartialEq, Serialize, Deserialize)]
pub struct ItemFade {
    pub length: Duration,
    pub curve: f64,
    pub shape: ItemFadeShape,
    /// if true - length is not counted.
    pub auto_fade_length: bool,
}
impl ItemFade {
    pub fn new(
        length: Duration,
        curve: f64,
        shape: ItemFadeShape,
        auto_fade_length: bool,
    ) -> Self {
        Self {
            length,
            curve,
            shape,
            auto_fade_length,
        }
    }
}

/// Item fade-in\fade-out shape.
///
/// # Note
///
/// Shape affects curve attribute of [ItemFade]
#[repr(i32)]
#[derive(
    Debug, Clone, Copy, PartialEq, Eq, IntEnum, Serialize, Deserialize,
)]
pub enum ItemFadeShape {
    Linear = 0,
    EqualPower = 1,
    SlightInner = 2,
    FastEnd = 3,
    FastStart = 4,
    SlowStartEnd = 5,
    Beizer = 6,
    Default = 7,
}
