...
use reaper_rx::{
    ActionRxHookPostCommand, ActionRxHookPostCommand2,
...
pub fn execute_integration_test(
    ...
    let builder =
        Reaper::load(*reaper_medium::Reaper::get().low().plugin_context());
    builder.setup();
...
impl TestControlSurfaceMiddleware {
    fn new() -> Self {
        Self {
            ...
            rx_middleware: ControlSurfaceRxMiddleware::new(
                Test::control_surface_rx().clone(),
            ),
...
struct RxSetup {
    control_surface_reg_handle: RegistrationHandle<
        MiddlewareControlSurface<TestControlSurfaceMiddleware>,
    >,
}
...
impl RxSetup {
    fn setup() -> RxSetup {
        ...
        RxSetup {
            control_surface_reg_handle: {
                let surface = MiddlewareControlSurface::new(
                    TestControlSurfaceMiddleware::new(),
                );
                session
    ...
    fn teardown(&self) {
        let mut session = Reaper::get().medium_session();
        unsafe {
            let _ = session.plugin_register_remove_csurf_inst(
                self.control_surface_reg_handle,
            );
        }
...
fn execute_next_step(
    ...
    if reaper_version_matches(&step) {
        let result = {
            ...
            .unwrap_or_else(|_| {
                Err(format!("Test [{}] panicked", step_name).into())
            });
            finished.complete();
            result
        };
